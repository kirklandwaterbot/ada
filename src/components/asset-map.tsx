...
type MappableFeature = AssetFeature | PlannedFeature;
...
export function AssetMap({ assets }: { assets: MtaAsset[] }) {
      ...
      .map((asset): AssetFeature | null => {
        ...
        return {
          ...
          properties: {
            ...
            line: formatSubwayLine(asset.subway_line),
  ...
  }, [allFeatures, allPlannedFeatures, collection, features, plannedCollection, plannedFeatures]);
  ...
  useEffect(() => {
    ...
    fitMapToFeatures(map, [...features, ...plannedFeatures]);
  }, [collection, features, plannedCollection, plannedFeatures]);
...
function popupCloseButtonHtml() {
  return `
    <button
      ...
      type="button"
    >
      <span class="material-symbols-rounded text-[18px] leading-none" aria-hidden="true">close</span>
    </button>
  `;
}
